...
import dev.antonio.request.credit.system.dto.CustomerUpdateDto
...
class CustomerResourceTest {
    ...
    fun `should create a customer and return 201 status`() {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post(URL).contentType(MediaType.APPLICATION_JSON)
            ...
            .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("antonio@email.com"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.income").value("1000.0"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.zipCode").value("000000"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.street").value("Rua da obra, 123"))
    ...
    fun `should not find customer with invalid id and return 400 status`() {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("$URL/$invalidId").accept(MediaType.APPLICATION_JSON))
            ...
            .andExpect(
                MockMvcResultMatchers.jsonPath("$.exception")
                    .value("class dev.antonio.request.credit.system.exception.BusinessException")
            )
            .andExpect(MockMvcResultMatchers.jsonPath("$.details[*]").isNotEmpty)
            .andDo(MockMvcResultHandlers.print())
    }
    ...
    fun `should update a customer and return 200 status`() {
        ...
        mockMvc.perform(MockMvcRequestBuilders.patch("$URL?customerId=${customer.id}").contentType(MediaType.APPLICATION_JSON)
            .content(valueAsString))
            .andExpect(MockMvcResultMatchers.status().isOk)
            .andExpect(MockMvcResultMatchers.jsonPath("$.firstName").value("AntonioUpdate"))
    ...
    fun `should not update a customer with invalid id and return 400 status`() {
        ...
        //when
        //then
        mockMvc.perform(MockMvcRequestBuilders.patch("$URL?customerId=$invalidId").contentType(MediaType.APPLICATION_JSON)
            .content(valueAsString))
            .andExpect(MockMvcResultMatchers.status().isBadRequest)
            .andExpect(MockMvcResultMatchers.jsonPath("$.title").value("Bad request. Consult the documentation"))
            .andExpect(MockMvcResultMatchers.jsonPath("$.timestamp").exists())
    ...
    private fun builderCustomerDto(
        ...
        email: String = "antonio@email.com",
        income: BigDecimal = BigDecimal.valueOf(1000.0),
        password: String = "1234",
        zipCode: String = "000000",
        street: String = "Rua da obra, 123",
    ) = CustomerDto(
        firstName = firstName,
        lastName = lastName,
        cpf = cpf,
    ...
    private fun builderCustomerUpdateDto(
        firstName: String = "AntonioUpdate",
